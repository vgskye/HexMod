...
public class WallScrollRenderer extends EntityRenderer<EntityWallScroll> {
	...
	public void render(
...

		{
			...
			if (PatternTextureManager.useTextures && wallScroll.points != null)
				PatternTextureManager.renderPatternForScroll(
						wallScroll.points.pointsKey,
						ps,
						bufSource,
						light,
						wallScroll.points.zappyPoints,
		...
		if (!PatternTextureManager.useTextures && wallScroll.points != null) {
			...
			// i swear, learning about these functions with asymptotes where slope != 0 is the most useful
			// thing
			...
			if (wallScroll.getShowsStrokeOrder()) {
				...
				theCoolerDrawSpot(mat, norm, light, verts, points.get(0), 2f / 3f * spotFrac, 0xff_5b7bd7);
			}
	...
	private static void vertexCol(
			Matrix4f mat, Matrix3f normal, int light, VertexConsumer verts, int col, Vec2 pos) {
		verts
				.vertex(mat, -pos.x, pos.y, 0)
				.color(col)
				.uv(0, 0)
				.overlayCoords(OverlayTexture.NO_OVERLAY)
				.uv2(light)
	...
	private static void theCoolerDrawLineSeq(
			Matrix4f mat,
			Matrix3f normalMat,
			int light,
			VertexConsumer verts,
		...
		for (int i = 2; i < points.size(); i++) {
			...
			var angle =
					(float) Mth.atan2(prev.x * next.y - prev.y * next.x, prev.x * next.x + prev.y * next.y);
	...
	private static void theCoolerDrawSpot(
			Matrix4f mat,
			Matrix3f normal,
			int light,
			VertexConsumer verts,
